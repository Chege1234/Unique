        ...
        <Card key={ticket.id} className="border-none shadow-md bg-green-50">
          <CardContent className="p-6">
            <div className="flex items-start gap-4">
              <div className={`w-10 h-10 ${ticket.status === 'skipped' ? 'bg-red-500' : 'bg-green-500'} rounded-full flex items-center justify-center flex-shrink-0`}>
              ...
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <Badge className={ticket.status === 'skipped' ? 'bg-red-500' : 'bg-green-500'}>{ticket.ticket_number}</Badge>
                </div>
                <div className="flex flex-wrap gap-4 text-sm text-gray-600">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    {ticket.status === 'skipped' ? 'Skipped' : 'Completed at'} {ticket.served_at ? format(new Date(ticket.served_at), 'h:mm a') : format(new Date(ticket.updated_date || ticket.created_at), 'h:mm a')}
                  </div>
                </div>
